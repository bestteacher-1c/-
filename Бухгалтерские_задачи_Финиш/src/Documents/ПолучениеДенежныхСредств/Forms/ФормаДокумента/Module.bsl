...
Процедура КорСчетПриИзмененииНаСервере()
	...
	Если КоличествоСубконто > 1 Тогда
		...
		Субконто2 =  Объект.КорСчет.ВидыСубконто.Получить(1).ВидСубконто;
		
		Объект.КорСубконто2 =
		Субконто2.ТипЗначения.ПривестиЗначение(Объект.КорСубконто2);
		...
		Если ТипЗнч(Объект.КорСубконто2) 
			...
			Массив.Добавить(Новый ПараметрВыбора("Отбор.Владелец", Субконто2));
