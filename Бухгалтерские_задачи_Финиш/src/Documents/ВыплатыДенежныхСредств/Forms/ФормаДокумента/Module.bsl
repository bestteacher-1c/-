...
Процедура КорСчетПриИзмененииНаСервере()
	...
	ДанныеСчета = БухгалтерияВызовСервера.ПолучитьСвойствоСчета(Объект.КорСчет);
	...
	Если (ДанныеСчета.КоличествоСубконто = 2) Тогда
		...
		Если ТипЗнч(Объект.КорСубконто2) 
		= Тип("СправочникСсылка.ЗначенияПользовательскихСубконто") Тогда
...
Процедура МестоХраненияДенежныхСредствПриИзмененииНаСервере()
	...
	Если (ПолучитьФункциональнуюОпцию("ВалютныйУчет", Новый Структура("ПФО_Организация", Объект.Организация)) = Истина) Тогда

		ДанныеСчета = БухгалтерияВызовСервера.ПолучитьСвойствоСчета(Объект.МестоХраненияДенежныхСредств.СчетУчета);
