...
Процедура ПродажаТовара(Отказ, Режим)
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|		ИЗ
		|			ВТТоварыДокумента КАК ВТТоварыДокумента),, Организация = &Организация
		...
		|	ЕСТЬNULL(ВТОстаткиВсе.КоличествоОстатокДт, 0) КАК КоличествоВсего,
		|	ЕСТЬNULL(ВТОстаткиВсе.СуммаОстатокДт, 0) КАК СуммаВсего
		|ИЗ
	...
	//--ПРОВЕРКА НАЛИЧИЯ УЧТЕННОГО ТМЦ ПО СЧЕТУ НА СКЛАДЕ
		...
		Если Нехватка > 0 Тогда
			...
			Сообщение.Текст = "Для проведения по данным бухгалтерского учета по счету "
				+ Выборка.СчетУчета + " не хватает номенклатуры """ + Выборка.Номенклатура
		...
		//=========================================================================
			...
			Если (Выборка.КоличествоВсего = 0) Тогда
				Себестоимость = 0;
			ИначеЕсли (Выборка.КоличествоВсего = Выборка.Количество) Тогда
				Себестоимость = Выборка.СуммаВсего;
			Иначе
				Себестоимость = Выборка.СуммаВсего / Выборка.КоличествоВсего
					* Выборка.Количество;
			КонецЕсли;
