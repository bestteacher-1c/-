...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ИспользоватьАвтосохранение = Истина;
	Объект.ПериодАвтосохранения = 2;
	
	ВключитьРежимВыбора();
	
	//РазмерШрифта = 14;
	
	Объект.ШрифтЗапрос = Элементы.ТекстЗапроса.Шрифт;
	Объект.ЦветЗапрос = Элементы.ТекстЗапроса.ЦветФона;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда 
		ТекстПредупреждения = НСтр("ru = 'Данные изменены. Если завершите работу, то изменения не сохраниться!'");
	КонецЕсли;
...
Процедура РезультатЗапросаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Объект.ДоступныеТипыДанных.СодержитТип(ТипРасшифровки) И РасшифровкаЯчейки <> Неопределено Тогда 
		ЗначениеДляПоказа = РасшифровкаЯчейки;
	ИначеЕсли ТипРасшифровки = Тип("Null") Тогда
		ЗначениеДляПоказа = "Null";
	ИначеЕсли ТипРасшифровки = Тип("Строка") Тогда
		ЗначениеДляПоказа = РасшифровкаЯчейки;
	ИначеЕсли ТипРасшифровки = Тип("Число") Тогда
		ЗначениеДляПоказа = РасшифровкаЯчейки;
	ИначеЕсли ТипРасшифровки = Тип("Булево") Тогда
		ЗначениеДляПоказа = ?(РасшифровкаЯчейки,"Истина","Ложь");
	ИначеЕсли ТипРасшифровки = Тип("Дата") Тогда
		ЗначениеДляПоказа = РасшифровкаЯчейки;
	ИначеЕсли ТипРасшифровки = Тип("ТаблицаЗначений") Тогда
		ЗначениеДляПоказа = РасшифровкаЯчейки;
	Иначе	
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	#ИначеЕсли ТонкийКлиент Тогда
		...
		ТЗ = Элементы.ТекстЗапроса.ВыделенныйТекст;
		Если ПустаяСтрока(ТЗ) Тогда
			
			Если СтрНайти(ТЗ," ") = 0 Тогда
				ТЗ = ТекстЗапроса.ПолучитьТекст();
			КонецЕсли;
		...
		ДопПараметры.Вставить("ТЗ", ТЗ);
		ДопПараметры.Вставить("ИндексТекущегоЗапроса", ИндексТекущегоЗапроса);
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияКонструктораЗапроса",ЭтотОбъект,ДопПараметры);
...
Процедура ПослеЗакрытияКонструктораЗапроса(Текст,ДопПараметры) Экспорт
	...
	ТЗ = Элементы.ТекстЗапроса.ВыделенныйТекст;
	Если ПустаяСтрока(ТЗ) И СтрНайти(ТЗ," ") = 0 Тогда
		ТекстЗапроса.УстановитьТекст(Текст);
	Иначе
		Элементы.ТекстЗапроса.ВыделенныйТекст = Текст;
	КонецЕсли;
	
	ДопПараметры.ТЗ					= ТекстЗапроса.ПолучитьТекст();
	
	ТекущийЗапрос		= Объект.Запросы.Получить(ДопПараметры.ИндексТекущегоЗапроса - 1);
	Если ТекущийЗапрос.Имя = ИмяЗапросаПоУмолчанию Тогда 
		ТекущийЗапрос.Имя	= ПолучитьИмяЗапроса(ДопПараметры.ТЗ);
	КонецЕсли;	
	ТекущийЗапрос.Текст = ДопПараметры.ТЗ;
	Модифицированность = Истина;
	ОбновитьФормуКлиент();
...
Процедура ВыполнитьЗапрос(Команда)
	...
	РезультатЗапроса.ТолькоПросмотр = Истина;
...
Процедура СкопироватьЗапрос(Команда)
	...
	ОткрытьФормуВыбораЗапроса(Неопределено);
...
Процедура СохранитьЗапросы()
	...
	Если ПроверитьВозможностьЗаписиСпискаЗапросов() = Ложь Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Автосохранение невозможно.'")
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, РезультатЗапроса, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, Текст, ТекстСообщения, Тонкий);
...
//
&НаСервере
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения, Тонкий = Ложь)
	...
	Результат = ОбъектОбработки().ВыполнитьЗапрос(Текст, МассивПараметров, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ПорядокОбхода, ТабличныйДокументРезультата, ВремяВыполнения, КоличествоСтрок, ТекстСообщения, ИспользованиеЧередования, МеткаЗапроса, Тонкий);
...
&НаКлиенте
Процедура СохранитьФайлЗапросаЗавершениеФрагмент(Знач Оповещение)
	
	Перем ДвоичныеДанные, Диалог, ИмяФайла, ТекстСообщения;
...
Процедура ЗаполнитьЗапросыИПараметрыИзОбъектаВнешнейОбработки(ОбъектОбработки)
	...
	Элементы.ТекстЗапроса.ЦветФона = Объект.ЦветЗапрос;
	Элементы.ТекстЗапроса.Шрифт = Объект.ШрифтЗапрос;
	...
	Для каждого ТекЗапрос Из ЗапросыОб Цикл
		...
		//Сизов
		ЭлементЗапроса.Заголовок			= ТекЗапрос.Заголовок;
		//Сизов
	...
	Для каждого ТекПараметр Из ПараметрыОб Цикл
		...
		ЭлементПараметр.ТипИзФайла = ТекПараметр.ТипИзФайла;
...
&НаКлиенте
Процедура ЦветЗапросПриИзменении(Элемент)
...
&НаКлиенте
Процедура ШрифтЗапросПриИзменении(Элемент)
...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВыбратьЗапросыИзФайла(Неопределено);
