#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Функция ЗаписатьЗапросыВФайлXML(знач Объект) Экспорт
		...
		ФайлXML.ЗаписатьАтрибут("name", XMLСтрока("%%Шрифт%%"));
		ФайлXML.ЗаписатьНачалоЭлемента("text");
		ФайлXML.ЗаписатьТекст( XMLСтрока(ЗначениеВСтрокуВнутр(ШрифтЗапрос)));
		...
		ФайлXML.ЗаписатьНачалоЭлемента("query");
		ФайлXML.ЗаписатьАтрибут("name", XMLСтрока("%%Цвет%%"));
		ФайлXML.ЗаписатьНачалоЭлемента("text");
		ФайлXML.ЗаписатьТекст( XMLСтрока(ЗначениеВСтрокуВнутр(ЦветЗапрос)));
		ФайлXML.ЗаписатьКонецЭлемента();
		ФайлXML.ЗаписатьНачалоЭлемента("parameters");
		ФайлXML.ЗаписатьКонецЭлемента();
		ФайлXML.ЗаписатьКонецЭлемента();
		...
		Для каждого ТекЗапрос Из Объект.Запросы Цикл
			ФайлXML.ЗаписатьНачалоЭлемента("query");
			ФайлXML.ЗаписатьАтрибут("name", XMLСтрока(ТекЗапрос.Имя));
			//Сизов
			//ФайлXML.ЗаписатьАтрибут("zagolovok", XMLСтрока(ТекЗапрос.Заголовок));
			//Сизов
			ФайлXML.ЗаписатьНачалоЭлемента("text");
				...
				Если ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "query" Тогда 
					Пока ФайлXML.ПрочитатьАтрибут() Цикл 
						Если ФайлXML.Имя = "name" Тогда
							//Сизов
							Если ФайлXML.Значение = "%%Шрифт%%" Тогда
								ЭтоШрифт = Истина;
								Прервать;
							Иначе
								ЭтоШрифт = Ложь;
							...
							Если ФайлXML.Значение = "%%Цвет%%" Тогда
								ЭтоЦвет = Истина;
								Прервать;
							Иначе
								ЭтоЦвет = Ложь;
							...
							//Сизов
							ДваСимволаЛево =  Лев(текЭлементЗапроса.Имя,2);
							...
							Если СтрНайти(ДваСимволаЛево,"0") <> 0
								...
								текЭлементЗапроса.Заголовок		=   Истина;
						...
						КонецЕсли;
						//Сизов
						//Если ФайлXML.Имя = "zagolovok" Тогда
						// Добавление запроса в таблицу.
						//текЭлементЗапроса 				= Запросы.Добавить();
						//текЭлементЗапроса.Идентификатор	= Новый УникальныйИдентификатор;
						//текЭлементЗапроса.Заголовок 	= XMLЗначение(Тип("Булево"),ФайлXML.Значение);
						//КонецЕсли;
					...
					Пока ФайлXML.Прочитать() Цикл 
						Если ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "text" Тогда 
							...
							Если ЭтоШрифт Тогда
								...
								ШрифтЗапрос = ЗначениеИзСтрокиВнутр(ФайлXML.Значение);
								
							ИначеЕсли ЭтоЦвет Тогда	
								
								ЦветЗапрос = ЗначениеИзСтрокиВнутр(ФайлXML.Значение);
									...
									Пока ФайлXML.ПрочитатьАтрибут() Цикл 
										...
										Если ФайлXML.Имя = "type" Тогда
											Попытка
												...
												текПараметрЗапроса.ТипИзФайла	= Ложь;
											Исключение
												ТипЭлемента				= Тип("Строка");
												текПараметрЗапроса.ТипИзФайла	= Истина;
											КонецПопытки;
			...
			Если ФайлXML.Имя = "item" И ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда 
				Пока ФайлXML.ПрочитатьАтрибут() Цикл
					Если ФайлXML.Имя = "type" Тогда
						...
						Попытка
							ТипЭлемента = Тип(ФайлXML.Значение);
							ПараметрЗапроса.ТипИзФайла = Ложь;
						Исключение
							ТипЭлемента = Тип("Строка");
							ПараметрЗапроса.ТипИзФайла = Истина;
						КонецПопытки;
						//Сизов
					КонецЕсли;
					Если ФайлXML.Имя = "value" Тогда
		...
		Пока ФайлXML.Прочитать() Цикл 
			Если ФайлXML.Имя = "item" И ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда 
				Пока ФайлXML.ПрочитатьАтрибут() Цикл
					...
					КонецЕсли;
				КонецЦикла;
				Если ТипЗнч(ЗначениеСсылки) = Тип("Строка") Тогда
					//Сизов
				    МВ = "Момент времени " ;
					//Сизов
				Иначе	
	...
	//
	Функция ВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, ВыводитьВТ,  ВыводитьИдентификатор, ПорядокОбхода, ТДРезультатаЗапроса, ВремяВыполнения, КоличествоСтрок, ТекстСообщения, ИспользованиеЧередования, МеткаЗапроса = "", Тонкий = Ложь) Экспорт
		...
		// Вывод.
		Успешно = ВывестиРезультатЗапросов(ТДРезультатаЗапроса, Запрос, МассивТекстов, МассивЗапросов, ПараметрыЗапроса, ВыводитьВТ, ВыводитьИдентификатор, ПорядокОбхода, КоличествоСтрок, ИспользованиеЧередования, МеткаЗапроса, Тонкий);
		Если Не Успешно Тогда 
			Если ВозможноОшибкаИзЗаТочкиСЗапятой(ТекстЗапроса) Тогда 
				Сообщить("Результат запроса не был выведен. Возможно не экранирована точка с запятой. Для экранирования точки с запятой используется обратный слеш -""\;""(см. справку)");
			Иначе
				Сообщить("Результат запроса не были выведен. Количество полученных данных не равно количеству запросов");
			КонецЕсли;
		КонецЕсли;
	...
	//
	Функция ВывестиРезультатЗапросов(ТДРезультатаЗапроса, Запрос, МассивТекстов, МассивЗапросов, ПараметрыЗапроса, ВыводитьВТ, ВыводитьИдентификатор, ПорядокОбхода, КоличествоСтрок, ИспользованиеЧередования, МеткаЗапроса = "", Тонкий = Ложь)
		...
		Для Индекс = 0 По КоличествоТекстовЗапросов - 1 Цикл
			...
			Если ПозицияПоместить <> 0 И ВыводитьВТ Тогда // Если временная таблица и выводить временные таблицы.
				...
				Если Результат <> Неопределено Тогда 
					ТД = ВывестиРезультатОдногоЗапроса(ИмяЗапроса, Результат, Свертка, ВыводитьИдентификатор, ПорядокОбхода, Ложь, КоличествоСтрокОдногоЗапроса, МассивШириныКолонок, ИспользованиеЧередования, Тонкий);
			...
			ИначеЕсли ПозицияПоместить = 0 Тогда // Если таблица не временная, то используется готовый результат.
				...
				ТД = ВывестиРезультатОдногоЗапроса(ИмяЗапроса, Результат, Свертка, ВыводитьИдентификатор, ПорядокОбхода, Иерархия, КоличествоСтрокОдногоЗапроса, МассивШириныКолонок, ИспользованиеЧередования, Тонкий);
				ТДРезультатаЗапроса.Вывести(ТД);
			КонецЕсли;	
	...
	//
	Функция ВывестиРезультатОдногоЗапроса(ИмяЗапроса, РезЗапроса, Открыта, ВыводитьИдентификатор, ПорядокОбхода, Иерархия, КоличествоСтрок, МассивШириныКолонок, ИспользованиеЧередования, Тонкий = Ложь)
		РезЗапроса 	  = ВыгрузкаРезультата(РезЗапроса, ПорядокОбхода, Иерархия, Тонкий);
	...
	//
	Функция ВыгрузкаРезультата(РезультатЗапроса, ПорядокОбхода, Иерархия, Тонкий = Ложь)
		...
		Если Тонкий Тогда
			...
			МассивИменКолонокТЗ = Новый Массив;
			
			Для каждого ТекКолонка Из ВыгруженноеЗначение.Колонки Цикл
				
				Если ТекКолонка.ТипЗначения = Новый ОписаниеТипов("ТаблицаЗначений") Тогда
					
					МассивИменКолонокТЗ.Добавить(ТекКолонка.Имя);
					Имя = ТекКолонка.Имя;
					ВыгруженноеЗначение.Колонки.Удалить(ТекКолонка);
					ВыгруженноеЗначение.Колонки.Добавить(Имя,Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(50,ДопустимаяДлина.Переменная)));
					
				КонецЕсли;
				
			КонецЦикла; 
			
			Для каждого ТекИмя Из МассивИменКолонокТЗ Цикл
				
				ВыгруженноеЗначение.ЗаполнитьЗначения("Результат запроса",ТекИмя);
